...
weight_U<-list()
for(i in 1:999){
m<-runif(1,min=log(0.10),max=log(40)) #ln of desired mean velocity
s<-runif(1,min=log(10),max=log(10))
...
data<- mutate(vel_patch, coverage = ifelse(u > 10, h, l)) #change accordingly
...
weight_U<-as.data.frame(as.numeric(weight_U))
colnames(weight_U)<- c("U")
plot(weight_u$u, weight_U$U,xlim=c(0,80))
